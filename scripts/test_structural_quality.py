...
"""
    ...
    def test_analyze_only_overlapping_functions_and_aggregate_deduplicate_order(self):
        functions = [
            quality.Function("z", 30, 30, 120, 11, 6),
            quality.Function("a", 2, 2, 101, 11, 6),
            quality.Function("a", 2, 2, 101, 11, 6),
    ...
    def test_threshold_equality_clean_and_unknown_files_counted(self):
        self.assertEqual(quality.exceeded(100, 15, 5), ())
        self.assertEqual(quality.exceeded(101, 15, 5), ("nloc",))
    ...
    def test_summaries_include_marker_and_wording(self):
        finding = quality.Finding("x.py", "fn", 2, 3, ("nloc",), 101, 1, 1)
        ...
        self.assertIn("- `x.py`\n  - `fn` · lines 2–3", text)
        self.assertIn("Function length: **101 NLOC** · limit: **100**", text)
