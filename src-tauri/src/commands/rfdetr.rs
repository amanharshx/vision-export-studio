...
fn helper_path() -> Result<PathBuf, String> {
    ...
    Ok(Path::new(manifest_dir).join("python").join("rfdetr_export_helper.py"))
}
...
pub async fn inspect_rfdetr_checkpoint(
    ...
    if !trust_confirmed {
        return Err("RF-DETR checkpoint inspection requires trusted checkpoint confirmation.".to_string());
    }
    if !Path::new(&checkpoint_path).exists() {
        return Err(format!("checkpoint path does not exist: {}", checkpoint_path));
    }
    ...
    if output.status.success() {
        ...
        Err(format!("RF-DETR inspect failed with exit code {:?}", output.status.code()))
    }
}
...
{"success":true,"class_symbol":"RFDETRSmall","family":"detection","size":"small","requires_plus":false,"is_legacy":false,"error":null}
